...
# ╔═╡ 94462e11-ed1d-475b-a39a-e2f76f7f908c
using OrdinaryDiffEq, DiffEqDevTools, Sundials, ParameterizedFunctions, Plots, ODE, ODEInterfaceDiffEq, LSODA

# ╔═╡ 65010bf2-eada-4a38-beea-9913782ee809
using LinearAlgebra

# ╔═╡ 8d6ee456-2b1b-4cbc-b554-274b39b054e9
gr()

# ╔═╡ 96a7106c-8276-41a9-99a0-bb235e01c3b7
prob = begin
	const k1=.35e0
	const k2=.266e2
	const k3=.123e5
	...
	u0[9]  = 0.01
	u0[17] = 0.007
	ODEProblem(ODEFunction(f, jac=fjac),u0,(0.0,60.0))
	
end

# ╔═╡ 091544e0-e59a-46c5-9331-88db39c68180
sol = solve(prob,CVODE_BDF(),abstol=1/10^14,reltol=1/10^14)

# ╔═╡ e7fb1bfd-d7bd-433b-b3d3-c8017b98fe7c
md"""
## Higher Tolerances
"""

# ╔═╡ 7bdaac06-ebc0-4225-9d17-2a5611d876e4
abstols = 1.0 ./ 10.0 .^ (6:9)

# ╔═╡ a25f06f5-fa7d-42e9-bd40-bef8477b35a9
reltols = 1.0 ./ 10.0 .^ (3:6)

# ╔═╡ a5dbc26c-167a-432d-9aca-fbf45b7afcc1
setups1 = [Dict(:alg=>Rosenbrock23()),
          Dict(:alg=>FBDF()),
          Dict(:alg=>QNDF()),
          Dict(:alg=>TRBDF2()),
          Dict(:alg=>CVODE_BDF()),
...
# ╔═╡ 61145857-c020-4bde-8f57-837523cb4e51
wp1 = WorkPrecisionSet(prob,abstols,reltols,setups1;verbose=false,
                      save_everystep=false,appxsol=TestSolution(sol),maxiters=Int(1e5),numruns=10)

# ╔═╡ 22b251cc-c95d-4d16-b482-95a41d262f7a
plot(wp1)

# ╔═╡ ba032ffb-8b53-42cf-855d-56aad28dcdc8
setups2 = [Dict(:alg=>Rosenbrock23()),
          Dict(:alg=>KenCarp5()),
          Dict(:alg=>KenCarp4()),
          Dict(:alg=>KenCarp3()),
...
# ╔═╡ 6314f97b-7aeb-4791-a204-df2f962e0cbc
wp2 = WorkPrecisionSet(prob,abstols,reltols,setups2;save_everystep=false,appxsol=TestSolution(sol),maxiters=Int(1e5),numruns=10)

# ╔═╡ a0620154-0540-46ff-9f97-45ea38be580b
plot(wp2)

# ╔═╡ 4721e0ce-812e-46c2-a8c4-5b5fa2a6301a
setups3 = [Dict(:alg=>Rosenbrock23()),
          Dict(:alg=>TRBDF2()),
          Dict(:alg=>ImplicitEulerExtrapolation()),
          Dict(:alg=>ImplicitHairerWannerExtrapolation()), # Diverges
		  Dict(:alg=>ImplicitHairerWannerExtrapolation(min_order = 3)),
...
# ╔═╡ 7438bf02-54c7-48e7-8cf5-b70dbbb0017c
wp3 = WorkPrecisionSet(prob,abstols,reltols,setups3;
                      save_everystep=false,appxsol=TestSolution(sol),maxiters=Int(1e5),numruns=10)

# ╔═╡ 598ea501-a050-4aa4-bb12-a15b706968f4
plot(wp3)

# ╔═╡ 7535e149-3cf0-4b3e-afa2-d9e2d6cb188e
md"""
## Low Tolerances
"""

# ╔═╡ fb4198cc-a74a-4449-b6cd-03eaf86d368f
labstols = 1.0 ./ 10.0 .^ (7:13)

# ╔═╡ 16331d5a-02d7-4b55-95ef-2a4a7223af19
lreltols = 1.0 ./ 10.0 .^ (4:10)

# ╔═╡ 52eef9d9-14ef-4f9b-849c-7916f8ad1046
lsetups1 = [
          #Dict(:alg=>FBDF()),
          Dict(:alg=>QNDF()),
          Dict(:alg=>Rodas4P()),
...
# ╔═╡ be950db8-ac99-4474-aacb-c6356434cd75
lwp1 = WorkPrecisionSet(prob,labstols,lreltols,lsetups1;error_estimator=:l2,
		save_everystep=false,appxsol=TestSolution(sol),maxiters=Int(1e5),numruns=10)

# ╔═╡ 764f2515-bf13-4926-bae2-abb62b422640
plot(lwp1)

# ╔═╡ 66a065fe-b552-415e-a2d9-4d4ce2a85f3f
lsetups2 = [Dict(:alg=>Kvaerno4()),
          Dict(:alg=>Kvaerno5()),
          Dict(:alg=>CVODE_BDF()),
          Dict(:alg=>KenCarp4()),
          Dict(:alg=>KenCarp47()),
...
# ╔═╡ 09d10765-e8a4-4f52-8a45-e2444a42c70d
lwp2 = WorkPrecisionSet(prob,labstols,lreltols,lsetups2;
                      save_everystep=false,appxsol=TestSolution(sol),maxiters=Int(1e5),numruns=10)

# ╔═╡ b28d4714-031b-4f15-a5d3-c84b989f2556
plot(lwp2)

# ╔═╡ e543257d-6097-43a9-ae33-8e4a83cf17ac
md"""
## Multi-threading
"""

# ╔═╡ 6f79401f-6c28-4b11-aa06-f4347cc0bdf7
msetups1 = [
...
# ╔═╡ 125cb094-28c8-4515-9e6e-a8ed16f62ead
names = ["unthreaded","threaded","Polyester"];

# ╔═╡ 879262e5-a3a5-4ffa-b0ee-4d25e47a4f72
mwp1 = WorkPrecisionSet(prob,abstols,reltols,msetups1;names=names,               save_everystep=false,appxsol=TestSolution(sol),maxiters=Int(1e5),numruns=10)

# ╔═╡ e01bbf9d-9999-489b-a1a4-24e3511f5056
plot(mwp1)
...
[compat]
DiffEqDevTools = "~2.28.0"
LSODA = "~0.7.0"
ODE = "~2.13.0"
ODEInterfaceDiffEq = "~3.10.1"
OrdinaryDiffEq = "~6.0.3"
ParameterizedFunctions = "~5.13.0"
Plots = "~1.25.4"
Sundials = "~4.8.0"
"""
...
[[deps.DiffEqDevTools]]
deps = ["DiffEqBase", "DiffEqNoiseProcess", "Distributed", "LinearAlgebra", "Logging", "NLsolve", "RecipesBase", "RecursiveArrayTools", "RootedTrees", "Statistics"]
git-tree-sha1 = "9b4dd4e3ed728bb88637075a5900f86d4133215a"
...
[[deps.LSODA]]
deps = ["BinDeps", "Compat", "DiffEqBase", "LSODA_jll", "Libdl", "LinearAlgebra", "Parameters", "Printf"]
git-tree-sha1 = "77d69af9f974da8bb18108b571f4e4b6616fe97b"
uuid = "7f56f5a3-f504-529b-bc02-0b1fe5e64312"
version = "0.7.0"

[[deps.LSODA_jll]]
deps = ["Artifacts", "JLLWrappers", "Libdl", "Pkg"]
git-tree-sha1 = "3a19962a454cf319151cee31219dd5eb3434a37d"
...
[[deps.ODEInterface]]
deps = ["Dates", "Libdl", "LinearAlgebra", "Markdown", "ODEInterface_jll", "Unicode"]
git-tree-sha1 = "5dfb3d7199de97cfdc81f1971a2727e628ff472b"
uuid = "54ca160b-1b9f-5127-a996-1867f4bc2a2c"
version = "0.5.0"

[[deps.ODEInterfaceDiffEq]]
deps = ["Compat", "DataStructures", "DiffEqBase", "FunctionWrappers", "LinearAlgebra", "ODEInterface", "Reexport"]
git-tree-sha1 = "69651775acfe4c5453058662ea6a84ef32994662"
uuid = "09606e27-ecf5-54fc-bb29-004bd9f985bf"
version = "3.10.1"

[[deps.ODEInterface_jll]]
deps = ["Artifacts", "CompilerSupportLibraries_jll", "JLLWrappers", "Libdl", "Pkg"]
git-tree-sha1 = "3021aa85a1cd566a533635246b7ed31a29c72c93"
uuid = "c771fb93-2439-5ece-a3b3-58fecce82101"
version = "0.0.1+0"
...
git-tree-sha1 = "7ac4d31a61b354c6f954a594eba05724079d8383"
...
[[deps.RootedTrees]]
deps = ["LinearAlgebra"]
git-tree-sha1 = "d7e2724f017038e4151209593d2df562e8704878"
...
git-tree-sha1 = "872d182f61a5d626e21cd970597cfb1856d8ef50"
