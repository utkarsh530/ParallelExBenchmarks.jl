...
abstols = 1.0 ./ 10.0 .^ (4:6)
reltols = 1.0 ./ 10.0 .^ (1:3)

setups = [  
            Dict(:alg=>ImplicitHairerWannerExtrapolation(threading = false,sequence=:harmonic)),
            Dict(:alg=>ImplicitHairerWannerExtrapolation(threading = OrdinaryDiffEq.PolyesterThreads(),sequence=:harmonic))
         ]

wp = WorkPrecisionSet(prob,abstols,reltols,setups;error_estimator=:l2,
                    save_everystep=false,maxiters=Int(1e5),numruns=10,names=["threading false", "polyester"])
